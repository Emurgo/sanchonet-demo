...
RUN direnv allow
CMD export UNSTABLE_LIB="true" && export UNSTABLE="true" && export NODE_CONFIG="state-demo/rundir/node-config.json" && export NODE_TOPOLOGY="state-demo/rundir/topology.json" && export SOCKET_PATH="/root/ipc/node.socket" && nohup nix run .#run-cardano-node  && nix develop .# --command just run-demo
